...
USE CMF_VARS_MOD,        ONLY: D2RIVELV
USE CMF_VARS_MOD,        ONLY: D1PTHFLW, D1PTHFLW_PRE, D2RIVDPH_PRE
USE CMF_VARS_MOD,        ONLY: D2SFCELV, D2STORGE, D1PTHFLWSUM
IMPLICIT NONE
!*** Local
...
DO IPTH=1, NPTHOUT  
  ...
  IF( D1PTHFLWSUM(IPTH)/=0._JPRB )THEN
    RATE= 0.05*min(D2STORGE(ISEQP,1),D2STORGE(JSEQP,1)) / abs(D1PTHFLWSUM(IPTH)*CMF_CONFIG%DT)  !! flow limit: 5% storage of upstream or downstream grid
