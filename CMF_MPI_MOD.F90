...
USE CMF_VARS_MOD,             ONLY: REGIONALL, REGIONTHIS, MPI_COMM_CAMA
!$ USE OMP_LIB
IMPLICIT NONE
!** local variables
!** MPI setting
...
IMPLICIT NONE
!* input/output
REAL(KIND=JPRM),INTENT(INOUT)   :: R2MAP(CMF_CONFIG%NX,CMF_CONFIG%NY)
!* local variable
...
#else
R2TMP(:,:)=CMF_PARAMS%RMIS
CALL MPI_AllReduce(R2MAP,R2TMP,CMF_CONFIG%NX*CMF_CONFIG%NY,MPI_REAL4,MPI_MIN,MPI_COMM_CAMA,ierr)
R2MAP(:,:)=R2TMP(:,:)
#endif
...
IMPLICIT NONE
!* input/output
REAL(KIND=JPRB),INTENT(INOUT)   :: D2MAP(CMF_CONFIG%NX,CMF_CONFIG%NY)
!* local variable
REAL(KIND=JPRB)                 :: D2TMP(CMF_CONFIG%NX,CMF_CONFIG%NY)
!================================================
! gather to master node
#ifdef IFS_CMF
CALL MPL_ALLREDUCE(D2MAP,CDOPER='MIN',KCOMM=MPI_COMM_CAMA,KERROR=ierr)
#else
D2TMP(:,:)=CMF_PARAMS%DMIS
#ifdef SinglePrec_CMF
CALL MPI_AllReduce(D2MAP,D2TMP,CMF_CONFIG%NX*CMF_CONFIG%NY,MPI_REAL4,MPI_MIN,MPI_COMM_CAMA,ierr)
#else
CALL MPI_AllReduce(D2MAP,D2TMP,CMF_CONFIG%NX*CMF_CONFIG%NY,MPI_REAL8,MPI_MIN,MPI_COMM_CAMA,ierr)
#endif
D2MAP(:,:)=D2TMP(:,:)
#endif
...
IMPLICIT NONE
!* input/output
REAL(KIND=JPRD),INTENT(INOUT)   :: P2MAP(CMF_CONFIG%NX,CMF_CONFIG%NY)
!* local variable
REAL(KIND=JPRD)                 :: P2TMP(CMF_CONFIG%NX,CMF_CONFIG%NY)
!================================================
! gather to master node
#ifdef IFS_CMF
CALL MPL_ALLREDUCE(P2MAP,CDOPER='MIN',KCOMM=MPI_COMM_CAMA,KERROR=ierr)
#else
P2TMP(:,:)=CMF_PARAMS%DMIS
CALL MPI_AllReduce(P2MAP,P2TMP,CMF_CONFIG%NX*CMF_CONFIG%NY,MPI_REAL8,MPI_MIN,MPI_COMM_CAMA,ierr)
P2MAP(:,:)=P2TMP(:,:)
#endif
...
#endif
DT_MIN=DT_OUT
WRITE(CMF_PARAMS%LOGNAM,'(A,2F10.2)') "ADPSTP (MPI_AllReduce): DT_LOC->DTMIN", DT_LOC, DT_MIN

END SUBROUTINE CMF_MPI_ADPSTP
!####################################################################

#endif
END MODULE CMF_MPI_MOD
