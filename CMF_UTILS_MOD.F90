...
USE CMF_NMLIST_MOD,     ONLY: CMF_FILES
...
USE CMF_VARS_MOD,            ONLY: I1SEQX,I1SEQY
USE CMF_NMLIST_MOD,     ONLY: CMF_CONFIG,CMF_PARAMS
IMPLICIT NONE
!* input/output
REAL(KIND=JPRB),INTENT(IN)      :: D2VEC(NSEQMAX,1)
REAL(KIND=JPRM),INTENT(OUT)     :: R2MAP(CMF_CONFIG%NX,CMF_CONFIG%NY)
!* local variable
INTEGER(KIND=JPIM),SAVE         ::  IX,IY,ISEQ
!$OMP THREADPRIVATE                (IX,IY)
!================================================
R2MAP(:,:) =  CMF_PARAMS%RMIS 
!$OMP PARALLEL DO
DO ISEQ=1,NSEQALL
  IX=I1SEQX(ISEQ)
  IY=I1SEQY(ISEQ)
...
!* input/output
REAL(KIND=JPRB),INTENT(IN)      :: D2VEC(NSEQMAX,1)
REAL(KIND=JPRB),INTENT(OUT)     :: D2MAP(CMF_CONFIG%NX,CMF_CONFIG%NY)
!* local variable
INTEGER(KIND=JPIM),SAVE         ::  IX,IY,ISEQ
!$OMP THREADPRIVATE                (IX,IY)
!================================================
D2MAP(:,:) = CMF_PARAMS%DMIS
!$OMP PARALLEL DO
DO ISEQ=1,NSEQALL
  IX=I1SEQX(ISEQ)
  IY=I1SEQY(ISEQ)
...
REAL(KIND=JPRD),INTENT(OUT)     :: P2MAP(CMF_CONFIG%NX,CMF_CONFIG%NY)
!* local variable
INTEGER(KIND=JPIM),SAVE         ::  IX,IY,ISEQ
!$OMP THREADPRIVATE                (IX,IY)
!================================================
P2MAP(:,:) = CMF_PARAMS%DMIS
!$OMP PARALLEL DO
DO ISEQ=1,NSEQALL
  IX=I1SEQX(ISEQ)
  IY=I1SEQY(ISEQ)
...
USE CMF_VARS_MOD,            ONLY: I1SEQX,I1SEQY
USE CMF_NMLIST_MOD,     ONLY: CMF_CONFIG
IMPLICIT NONE
!* input/output
REAL(KIND=JPRM),INTENT(IN)      :: R2TEMP(CMF_CONFIG%NX,CMF_CONFIG%NY)
...
IMPLICIT NONE
!* input/output
REAL(KIND=JPRB),INTENT(IN)      :: D2TEMP(CMF_CONFIG%NX,CMF_CONFIG%NY)
REAL(KIND=JPRB),INTENT(OUT)     :: D2VAR(NSEQMAX,1)
!* local variable
INTEGER(KIND=JPIM),SAVE         :: IX,IY, ISEQ
!$OMP THREADPRIVATE               (IX,IY)
...
IMPLICIT NONE
!* input/output
REAL(KIND=JPRD),INTENT(IN)      :: P2TEMP(CMF_CONFIG%NX,CMF_CONFIG%NY)
...
IMPLICIT NONE
!* input/output
INTEGER(KIND=JPIM),INTENT(IN)   :: I2TEMP(CMF_CONFIG%NX,CMF_CONFIG%NY)
...
USE CMF_NMLIST_MOD,     ONLY: CMF_TIME
IMPLICIT NONE
! local
...
!============================
IF ( YYYY .LT. CMF_TIME%YYYY0) THEN
  WRITE(CMF_FILES%LOGNAM,*) 'DATE2MIN: YYYY .LT. YYYY0: Date Problem', YYYY,CMF_TIME%YYYY0
  STOP
ENDIF
IF ( MM.LT.1 .or. MM .GT. 12 ) THEN
  WRITE(CMF_FILES%LOGNAM,*) 'DATE2MIN: MM:    Date Problem', YYYYMMDD, HHMM
  STOP
ENDIF
IF ( DD.LT.1 .or. DD .GT. IMDAYS(YYYY,MM)) THEN
  WRITE(CMF_FILES%LOGNAM,*) 'DATE2MIN: DD:    Date Problem', YYYYMMDD, HHMM
  STOP
ENDIF
IF ( HH.LT.0 .or. HH .GT. 24) THEN
  WRITE(CMF_FILES%LOGNAM,*) 'DATE2MIN: HH:    Date Problem', YYYYMMDD, HHMM
  STOP
ENDIF
IF ( MI.LT.0 .or. MI .GT. 60) THEN
  WRITE(CMF_FILES%LOGNAM,*) 'DATE2MIN: MI:    Date Problem', YYYYMMDD, HHMM
  STOP
ENDIF
...
!! days in month
USE CMF_NMLIST_MOD,     ONLY: CMF_OPTIONS
IMPLICIT NONE
...
!================================================
IMDAYS=ND(IMON)
IF ( IMON == 2 .and. CMF_OPTIONS%LLEAPYR ) THEN
...
!================================================
IF ( STATUS /= 0 ) THEN
  WRITE(CMF_FILES%LOGNAM,*)  TRIM(NF90_STRERROR(STATUS))
  IF( PRESENT(STRING) ) WRITE(CMF_FILES%LOGNAM,*) TRIM(STRING)
  WRITE(CMF_FILES%LOGNAM,*) 'PROGRAM STOP ! '
  STOP 10
ENDIF
...
subroutine sed_diag_average
  use CMF_VARS_MOD,             only: d2rivout_sed, d2rivout_sed, d2rivvel_sed,sadd_riv
  implicit none
...
subroutine sed_diag_reset
  ...
  USE CMF_NMLIST_MOD,          ONLY: CMF_SED
  use CMF_VARS_MOD,             only: d2rivsto_pre, d2rivout_sed, d2rivvel_sed, &
                                     sadd_riv, sadd_out
  implicit none
  ...
  sadd_out = sadd_out + CMF_SED%sedDT
