...
USE CMF_VARS_MOD,          ONLY: P2RIVSTO, P2FLDSTO, D2FLDOUT, D2FLDOUT_PRE
USE CMF_VARS_MOD,          ONLY: D2STORGE
!
USE CMF_OUTFLW_MOD,        ONLY: CMF_CALC_OUTFLW, CMF_CALC_INFLOW
USE CMF_PTHOUT_MOD,        ONLY: CMF_CALC_PTHOUT
USE CMF_STONXT_MOD,        ONLY: CMF_CALC_STONXT
USE CMF_DIAG_MOD,          ONLY: CMF_DIAG_RESET_ADPSTP, CMF_DIAG_AVEMAX_ADPSTP, CMF_DIAG_GETAVE_ADPSTP
! optional
USE CMF_OUTFLW_MOD,        ONLY: CMF_CALC_OUTFLW_KINEMIX, CMF_CALC_OUTFLW_KINE,CMF_CALC_OUTINS
USE CMF_DAMOUT_MOD,        ONLY: CMF_DAMOUT_CALC, CMF_DAMOUT_WATBAL
USE CMF_LEVEE_MOD,         ONLY: CMF_LEVEE_OPT_PTHOUT

USE CMF_VARS_MOD,          ONLY: LLAKEIN
USE CMF_LAKEIN_MOD,        ONLY: CMF_CALC_LAKEIN, CMF_LAKEIN_AVE
...
!=== 1. Calculate river discharge 
  ...
  IF ( CMF_OPTIONS%LKINE  ) THEN
    CALL CMF_CALC_OUTFLW_KINE       !!  OPTION: kinematic
  ELSEIF( CMF_OPTIONS%LSLPMIX ) THEN
...
USE CMF_VARS_MOD,        ONLY: D2NXTDST
USE CMF_VARS_MOD,        ONLY: NSEQALL,NSEQRIV,I2MASK
USE CMF_VARS_MOD,       ONLY: D2RIVDPH
#ifdef UseMPI_CMF
USE CMF_MPI_MOD,   ONLY: CMF_MPI_ADPSTP
#endif
IMPLICIT NONE
! MPI setting
! SAVE for OpenMP
...
DO ISEQ=1, NSEQRIV
  IF( I2MASK(ISEQ,1)==0 )THEN
    ...
    DT_MIN=min( DT_MIN, CMF_PARAMS%PCADP*DDST * (CMF_PARAMS%PGRV*DDPH)**(-0.5) )
  ENDIF
END DO
!$OMP END PARALLEL DO
...
CMF_CONFIG%DT=DT_DEF * REAL(NT)**(-1.)

IF( NT>=2 ) WRITE(CMF_FILES%LOGNAM,'(A15,I4,3F10.2)') "ADPSTP: NT=",NT, DT_DEF, DT_MIN, CMF_CONFIG%DT
...
USE CMF_VARS_MOD,            ONLY: P0GLBSTOPRE, P0GLBSTONXT, P0GLBSTONEW,P0GLBRIVINF,P0GLBRIVOUT !! dischrge calculation
...
! ================================================
PKMIN=INT ( KMIN + IT*CMF_CONFIG%DT/60_JPRB )
...
WRITE(CMF_FILES%LOGNAM,'(I4.4,4(A1,I2.2),I6,a6,3F12.3,G12.3,2x,2F12.3,a6, 2F12.3,G12.3,3F12.3)') &
...
USE CMF_VARS_MOD,            ONLY: D2RIVOUT,     D2FLDOUT,     P2FLDSTO
USE CMF_VARS_MOD,            ONLY: D2RIVOUT_PRE, D2FLDOUT_PRE, D2FLDSTO_PRE, D2RIVDPH_PRE
USE CMF_VARS_MOD,            ONLY: D1PTHFLW, D1PTHFLW_PRE
USE CMF_VARS_MOD,            ONLY: D2RIVDPH
IMPLICIT NONE
! ================================================
